...
Reads dependencies via `port info`. Emits the wave list as JSON on stdout:
    [[{"label": "w0c0", "ports": "llvm-22", "deps": {"llvm-22": ""}}],
     [{"label": "w1c0", "ports": "clang-22 mlir-22",
       "deps": {"clang-22": "llvm-22", "mlir-22": "llvm-22"}}], ...]
where `deps` maps each subport to its changed dependencies placed in *earlier*
waves: those are exactly the PR-built archives the build job expects to
restore, so it can skip a subport (with a clear notice) when a dependency
failed to produce its archive instead of pointlessly rebuilding the failed
dependency from source. Dependencies in the same wave (possible after
collapsing) are not listed: they have no archive by design and fall back to a
source build.
When run inside GitHub Actions (GITHUB_OUTPUT set) the same JSON is written as
`waves` plus `num_waves` and `wave_names`.
"""
...
def changed_blockers(subports):
    """Map each subport to the set of changed subports it depends on."""
    changed = set(subports)
    return {p: (port_deps(p) & changed) - {p} for p in subports}


def compute_waves(subports, blockers):
    """Topologically sort `subports` into dependency-ordered waves.

    Only edges between two changed subports are considered (`blockers`).
    Raises ValueError if a dependency cycle is detected among the changed
    subports.
    """
    waves = []
    placed = set()
...
def chunk_waves(waves, blockers, chunk_size):
    """Split each wave into labelled chunks of at most chunk_size subports.

    Each chunk carries a `deps` map: subport -> its changed dependencies from
    strictly earlier waves, i.e. the PR-built archives the build job must find
    restored before building that subport.
    """
    wave_index = {p: w for w, wave in enumerate(waves) for p in wave}
    result = []
    for w, wave in enumerate(waves):
        chunks = []
        for c in range(0, len(wave), chunk_size):
            ports = wave[c : c + chunk_size]
            chunks.append(
                {
                    ...
                    "deps": {
                        p: " ".join(
                            sorted(d for d in blockers[p] if wave_index[d] < w)
                        )
                        for p in ports
                    },
                }
...
def main(argv):
    ...
    if not subports:
        ...
        blockers = changed_blockers(subports)
        waves = chunk_waves(
            collapse_waves(compute_waves(subports, blockers), args.max_waves),
            blockers,
            args.chunk_size,
        )
