...
Usage:
    compute-graph.py SUBPORT [SUBPORT ...]

Emits the entry list as JSON on stdout:
    [{"port": "llvm-22", "waits": ""},
     {"port": "clang-22", "waits": "llvm-22"},
     {"port": "flang-22", "waits": "clang-22 llvm-22 mlir-22"}, ...]
and, when run inside GitHub Actions (GITHUB_OUTPUT set), writes the same JSON
as `entries` plus `count`.
"""
...
def main(argv):
    ...
    if github_output:
        with open(github_output, "a") as fh:
            fh.write(f"entries={json.dumps(entries)}\n")
